...
def get_motion_loss(inte_state, label, confs, ts=None, rot=None):
    ...
    if confs.obsersup:
        ...
        _, vel = retrieve_from_obser(ts, rot, inte_state['net_vel'], label[:, :1, :])
    else:
        vel, obs_vel = inte_state['net_vel'], get_observable_label(ts, rot, inte_state['net_vel'])
...
def get_motion_RMSE(inte_state, label, confs, ts=None, rot=None):
    ...
    if confs.obsersup:
        assert ts.shape[1] == label.shape[1]
        obs_vel = inte_state['net_vel']
        _, abs_vel = retrieve_from_obser(ts, rot, inte_state['net_vel'], label[:, :1, :])
    else:
        abs_vel, obs_vel = inte_state['net_vel'], get_observable_label(ts, rot, inte_state['net_vel'])
